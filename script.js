  ...
  .then((txs) => {
    ...
    for (let tx of txs.data) {   ///////////////    tx を pageSize の回数繰り返す ///////////////////
         ...
         if (check_netType === 'N'){   // MAINNET の場合          
           ...
           dom_signer_address.innerHTML = `<div class="copy_container"><font color="#2f4f4f">From : ${tx.signer.address.address}</font><small><input type="image" src="src/copy.png" height="25px" id="${tx.signer.address.address}" onclick="Onclick_Copy(this.id);" class="copy_bt" /></small></div>`;    //  送信者 アドレス
  ...
  .then((txs) => {
    ...
    for (let tx of txs.data) {   ///////////////    tx を pageSize の回数繰り返す ///////////////////
         ...
         if (check_netType === 'N'){   // MAINNET の場合          
           ...
           dom_signer_address.innerHTML = `<div class="copy_container"><font color="#2f4f4f">From : ${tx.signer.address.address}</font><small><input type="image" src="src/copy.png" height="25px" id="${tx.signer.address.address}" onclick="Onclick_Copy(this.id);" class="copy_bt" /></small></div>`;    //  送信者 アドレ
