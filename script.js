     ...
     　　　 dom_tx.appendChild(dom_date);                     //　dom_date　をdom_txに追加           
	   dom_tx.appendChild(dom_hash);                      // dom_hash をdom_txに追加
	    ...
	    if (tx.recipientAddress.address === undefined){
		     ...
		     let Namejson =JSON.stringify(namespacesNames);
		      console.log("Namejson=",Namejson.name); 
		     dom_recipient_address.innerHTML = `<font color="#2f4f4f">To :   ${Namejson.name}</font>`; //  文字列の結合　   宛先
                })(); // async() 
	    }else{   
