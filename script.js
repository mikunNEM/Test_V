          ...
          for(let i=0; i<tx.mosaics.length; i++){  //モザイクの数だけ繰り返す
            ...
            if (tx.mosaics.length !== 0){ //モザイクが空でない(モザイク有りの場合)
               (async() => {
                 ...
                 if(tx.signer.address.address === address.address) {  // signerとウォレットのアドレスが同じかどうかで絵文字の表示と色を変える
                     if ([mosaicNames][0][0].names.length !==0){  // ネームスペースがある場合
                         dom_mosaic[i].innerHTML = `<font color="#FF0000">Mosaic :　<big><strong>${[mosaicNames][0][0].names[0].name}</strong></big>　<small>(${tx.mosaics[i].id.id.toHex()})</small></font>`; 
                         ...
                         dom_mosaic.innerHTML = `<font color="#FF0000">Mosaic :　<small>(${tx.mosaics[i].id.id.toHex()})</small></font>`;
                          console.log("Namespace無し",tx.mosaics[i].id.id.toHex())////////////////////////////////////
                     }    
                         dom_amount[i].innerHTML = `<font color="#FF0000" size="+1">💁‍♀️➡️💰 :<i><big><strong> ${(parseInt(tx.mosaics[i].amount.toHex(), 16)/(10**div)).toLocaleString(undefined, { maximumFractionDigits: 6 })} </big></strong><i></font>`;     // 　数量               
                 }else {                                         //  signer とウォレットアドレスが違う場合
                     if ([mosaicNames][0][0].names.length !==0){ // ネームスペースがある場合
                         console.log([mosaicNames][0][0].names[0].name)/////////////////////////////// 
                         dom_mosaic[i].innerHTML = `<font color="#008000">Mosaic :　<big><strong>${[mosaicNames][0][0].names[0].name}</strong></big>　<small>(${tx.mosaics[i].id.id.toHex()})</small></font>`; 
                     }else{ 　　　　　　　　　　　　　　　　　　　　　  // ネームスペースがない場合
                         dom_mosaic[i].innerHTML = `<font color="#FF0000">Mosaic :　<small>(${tx.mosaics[i].id.id.toHex()})</small></font>`;
                          console.log("Namespace無し",tx.mosaics[i].id.id.toHex())////////////////////////////////////
                     }
                         dom_amount[i].innerHTML = `<font color="#008000" size="+1">💰➡️😊 :<i><big><strong> ${(parseInt(tx.mosaics[i].amount.toHex(), 16)/(10**div)).toLocaleString(undefined, { maximumFractionDigits: 6 })} </big></strong><i></font>`;     // 　数量          
               ...
               if(tx.signer.address.address === address.address) {  // 送信アドレスとウォレットのアドレスが同じかどうかで絵文字の表示と色を変える
                  dom_mosaic[i].innerHTML = `<font color="#FF0000">Mosaic : No mosaic</font>`;     // No mosaic
                  dom_amount[i].innerHTML = `<font color="#FF0000">💁‍♀️➡️💰 : </font>`;     // 　数量      
               }else {
         　        　dom_mosaic[i].innerHTML = `<font color="#008000">Mosaic : No mosaic</font>`;     // No mosaic
                    dom_amount[i].innerHTML = `<font color="#008000">💰➡️😊 : </font>`;     // 　数量               
               }   
            }                                       
