     ...
     　　　 dom_tx.appendChild(dom_date);                     //　dom_date　をdom_txに追加
		...
		/*  setTimeout(() => {	 	 
		   window.SSS.setEncryptedMessage(      // メッセージを復号
                 ...
                 dom_message.innerHTML = `<font color="#ff00ff">< Encrypted Message ></font><font color="#4169e1"></br><input type="button" id="enc_message" onclick="Onclick_Decryption();"/>${tx.message.payload}</div></font>`;     // 　メッセージ    
            }else{          // 平文の場合
